...
pub const MINING_REWARD: f64 = 452.98;
// TODO: Assign the current block height
pub const CURRENT_BLOCK_HEIGHT: u64 = 951418;
...
pub fn generate_address(prefix: &str) -> String {
    ...
    if prefix_len >= 32 {
        return prefix.chars().take(32).collect();
    }

    let suffix_len = 32 - prefix_len;
    let mut range = rand::thread_rng();
    let suffix: String = (0..suffix_len)
        .map(|_| {
            let idx = range.gen_range(0..characters.len());
            characters[idx]
        })
        .collect();
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    if height < 0 {
        return (false, "negative".to_string());
    }
    if height > 1_000_000 {
...
pub fn create_utxo(
    ...
    new_map.insert("txid".to_string(), txid.to_string());
    new_map.insert("vout".to_string(), vout.to_string());
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    let bytes = hex::decode(version_hex).map_err(|e| format!("Hex decode error: {}", e))?;
